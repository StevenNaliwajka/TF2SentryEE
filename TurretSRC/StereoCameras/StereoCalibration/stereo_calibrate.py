...
def _assert_directories_same(left_stereo_path: Path, right_stereo_path: Path, ignore_dotted_files: bool = False) -> \
        list[str]:
    """
...
class CameraCalibrationResults:
    ...
    Args:
        ...
        old_intrinsic_matrix: the original matrix containing intrinsic values for the matrix such as focal length
        ...
        new_intrinsic_matrix: the corrected intrinsic matrix generated by `cv2.getOptimalNewCameraMatrix()` based on the
            free scaling paramter
        rotation_vector (Sequence): The triple describing the camera rotation (rx,ry,rz) in radians around that
            specific axis
        translation_vector (Sequence): The triple describing the camera translation from the world origin frame in the
            (x,y,z) format. The unit will be the same as the object points that you used for calibration.
            (Probably millimeters if you passed in square_size_mm)
            Note: this is sensitive to how you define your world origin. If you define your world origin as your
            camera, this will be (0,0,0).
        distortion_coeffs (np.ndarray): the 5-tuple containing information about how the camera was distorted.
            The format is (k1,k2,p1,p2,k3)
        image_size (np.ndarray) a double containing the image dimensions in a (height, width) format
        valid_roi (Sequence[int]) a 4-tuple rectangle defining the all-good-pixels in the undistorted image.
        rmse (float): Root Mean Squared Error: how accurate the camera calibration was. Anything under 1 is very good.
    """
    image_points: list[np.ndarray]
    old_intrinsic_matrix: np.ndarray
    new_intrinsic_matrix: np.ndarray
    rotation_vector: np.ndarray
    translation_vector: np.ndarray
    distortion_coeffs: np.ndarray
    image_size: np.ndarray
    valid_roi: Sequence[int]
    rmse: float

    def get_calibration_info(self) -> \
            tuple[
                list[np.ndarray], np.ndarray, np.ndarray, np.ndarray,
                np.ndarray, np.ndarray, np.ndarray, Sequence[int], float
            ]:
        """
        ...
        Returns:
            a tuple containing (image_points, intrinsic_matrix, rotation_vector, translation_vector, dist_coeffs,
            img_size, valid_roi, rmse) in that order.
        """
        return (self.image_points, self.old_intrinsic_matrix, self.new_intrinsic_matrix, self.rotation_vector,
                self.translation_vector, self.distortion_coeffs, self.image_size, self.valid_roi, self.rmse)
...
def calibrate_both_cameras(
    ...
    # This also implicitly sets the world origin frame to the top left of the checkerboard.
    ...
    inv_left_shape: np.ndarray = left_shape[::-1]
    inv_right_shape: np.ndarray = right_shape[::-1]

    # Note that we return the rotation vector instead of a rotation matrix. This is because openCV
    # converted the rotation matrix into a rotation vector using cv2.Rodrigues()

    # noinspection PyTypeChecker
    rmse_l, old_intrinsic_l, dist_l, rot_vect_l, trans_vect_l = cv2.calibrateCamera(obj_pts, img_pts_l, inv_left_shape,
                                                                                    None, None)
    new_intrinsic_l, valid_pix_roi_l = cv2.getOptimalNewCameraMatrix(old_intrinsic_l, dist_l, inv_left_shape, 1,
                                                                     inv_left_shape)

    # noinspection PyTypeChecker
    rmse_r, old_intrinsic_r, dist_r, rot_vect_r, trans_vect_r = cv2.calibrateCamera(obj_pts, img_pts_r, inv_right_shape,
                                                                                    None, None)
    new_intrinsic_r, valid_pix_roi_r = cv2.getOptimalNewCameraMatrix(old_intrinsic_r, dist_r, inv_right_shape, 1,
                                                                     inv_right_shape)
    left_camera_info: CameraCalibrationResults = \
        CameraCalibrationResults(
            image_points=img_pts_l,
            old_intrinsic_matrix=old_intrinsic_l,
            new_intrinsic_matrix=new_intrinsic_l,
            rotation_vector=rot_vect_l[0],
            translation_vector=trans_vect_l[0],
            distortion_coeffs=dist_l,
            image_size=inv_left_shape,
            valid_roi=valid_pix_roi_l,
            rmse=rmse_l
        )

    right_camera_info: CameraCalibrationResults = \
        CameraCalibrationResults(
            image_points=img_pts_r,
            old_intrinsic_matrix=old_intrinsic_r,
            new_intrinsic_matrix=new_intrinsic_r,
...
def save_all_results(
    ...
    img_pts_l, int_mtx_l, new_int_mtx_l, rot_l, trans_l, dist_l, left_img_size, valid_roi_l, rmse_l = left_camera_info.get_calibration_info()
    img_pts_r, int_mtx_r, new_int_mtx_r, rot_r, trans_r, dist_r, right_img_size, valid_roi_r, rmse_r = right_camera_info.get_calibration_info()

    np.savez_compressed(left_res_path,
                        image_points=np.stack(img_pts_l),  # Convert the list of points into a nxk matrix.
                        old_intrinsic_matrix=int_mtx_l,
                        new_intrinsic_matrix=new_int_mtx_l,
                        distortion_coeffs=dist_l,
                        rotation_matrix=rot_l,
                        translation_vector=trans_l,
                        image_size=left_img_size,
                        valid_roi=valid_roi_l,
                        rmse=rmse_l,
    ...
    np.savez_compressed(right_res_path,
                        image_points=np.stack(img_pts_r),
                        old_intrinsic_matrix=int_mtx_r,
                        new_intrinsic_matrix=new_int_mtx_l,
                        distortion_coeffs=dist_r,
                        rotation_matrix=rot_r,
                        translation_vector=trans_r,
                        image_size=right_img_size,
                        valid_roi=valid_roi_r,
                        rmse=rmse_r,
    ...
    if "left_camera_info" in requested_results:
        with np.load(left_res_path) as data:
            results["left_camera_info"] = CameraCalibrationResults(
                data["image_points"], data["old_intrinsic_matrix"],
                data["new_intrinsic_matrix"],
                data["rotation_matrix"], data["translation_vector"],
                data["distortion_coeffs"], data["image_size"], data["valid_roi"], data["rmse"]
            )
...
def get_focal_lengths_px(
    ...
    return (
        (left_cam_info.new_intrinsic_matrix[0][0], left_cam_info.new_intrinsic_matrix[1][1]),
        (right_cam_info.new_intrinsic_matrix[0][0], right_cam_info.new_intrinsic_matrix[1][1])
    )
